...
class AGuhitSupabaseUser extends BaseAuthUser {
  AGuhitSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> aGuhitSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = AGuhitSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
